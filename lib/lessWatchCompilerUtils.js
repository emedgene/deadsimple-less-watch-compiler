...
define(function ( require ) {
        ...
        var lessWatchCompilerUtilsModule = {
            ...
            compileCSS: function(file, test){
                ...
                var command = lessWatchCompilerUtilsModule.config.lessc+sourceMap+minifiedFlag +' '+file.replace(/\s+/g,'\\ ')+' '+lessWatchCompilerUtilsModule.config.outputFolder+ '/' +file.replace(/.less$/g,'')+(lessWatchCompilerUtilsModule.config.minified ? '.min' : '')+'.css';
                // Run the command
                if (!test)
